...
sized to the largest box that uses it, across the bike, the scooter and the
one-wheel:

  material  offset (u,v)   region w x h   notes
  --------  ------------   ------------   ------------------------------------
  TYRE      (0, 0)         12 x 8         bike + scooter wheels
  FRAME     (16, 0)        12 x 20        bike tubes, scooter stem
  ACCENT    (32, 0)        26 x 10        saddles, decks, one-wheel foot pads
  METAL     (64, 0)        16 x 5         scooter fender, small hardware
  FATTYRE   (88, 0)        18 x 8         the one-wheel's fat slick
  SHELL     (0, 28)        24 x 8         one-wheel deck shells + bumpers
  RAIL      (32, 28)       22 x 12        one-wheel side rails

The last three were added rather than growing TYRE/FRAME in place: the original
four are boxed in by their neighbours, and shade() derives its gradient from the
region's own w/h, so resizing one would silently re-shade every part already
using it -- including on models the owner has already signed off.

Run:  python tools/gen_skins.py
Writes the entity PNGs and, with --preview, an 8x scaled PNG per variant to
...
PALETTES = {
    ...
    # one-wheel: matte-black shell and bumpers, charcoal grip pads, a fat black slick, and
    # cyan-anodised side rails. The rails carry the whole colour identity on purpose -- a board this
    # size is otherwise black-on-black at Minecraft's scale and would read as an unlit blob.
    "onewheel": {
        "tyre":    (22, 22, 25),
        "fattyre": (22, 22, 25),
        "frame":   (30, 31, 35),
...
# Note the one-wheel is absent: it is a standalone device that never joins the fleet
# (BikeVariant#usesStations), so there is no share livery to paint for it, and BikeVariant
# .shareTexture() falls back to its personal skin rather than asking for one that isn't here.
for _base in ("bike", "ebike", "scooter", "scooter_fast"):
    PALETTES["share_" + _base] = SHARE_LIVERY

# Every skin carries every region, whether or not its model uses all of them: the atlas layout is
# shared by all variants, and painting a rect a model never samples costs nothing while leaving one
# unpainted would show as a hole the day a model grows into it. A palette that doesn't name one of
# the newer materials inherits it from its closest sibling.
_MATERIAL_FALLBACKS = {"fattyre": "tyre", "shell": "frame", "rail": "metal"}
for _palette in PALETTES.values():
    for _material, _inherit_from in _MATERIAL_FALLBACKS.items():
        _palette.setdefault(_material, _palette[_inherit_from])
...
def shade(base, x, w, y, h, mat="frame"):
    ...
    pass paints in the character the flat colour can't carry on its own:
      ...
      fattyre - a slick: bright rim, fine lengthwise sipes, no chunky lugs
      frame   - a rounded-metal specular highlight near the left, weld seam mid-tube
      shell   - matte plastic: near-flat, one soft top sheen, no specular
      rail    - anodised metal: lengthwise brushed streaks, bright top edge
    ...
    if mat == "tyre":
        ...
        # A go-kart-style slick: no tread blocks, just a bright crown and fine siping. Kept flatter
        # than "tyre" so the extra width reads as one broad surface rather than a knobbly one.
        if y == 0:
            f *= 1.36                            # bright crown edge
        elif y % 4 == 0:
            f *= 0.88                            # fine lengthwise sipe
        else:
            f *= 1.04
    elif mat == "frame":
        ...
        # Moulded matte plastic: deliberately the flattest material here. A specular highlight would
        # make it read as painted metal, which is exactly what a one-wheel's shell is not.
        if y <= 1:
            f *= 1.10                            # soft top sheen
        f *= 1.02 if (x + y) % 2 == 0 else 0.98  # very fine moulding grain
    elif mat == "rail":
        # Anodised aluminium extrusion: brushed along its length, with a bright machined top edge.
        if y == 0:
            f *= 1.30                            # machined top edge catching the light
        f *= 1.10 if y % 3 == 0 else (0.92 if y % 3 == 1 else 1.0)   # lengthwise brush streaks
        if x == w - 1:
            f *= 0.84                            # shaded far edge, so the rail reads as round-ish
    elif mat == "accent":
        if y <= 1:
